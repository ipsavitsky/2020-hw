...
graph upload_graph(char *filename) {
    ...
    for (int i = 0; i < struct_size; ++i) {
        ...
        assert(new_arr[i].data != NULL);
        if (fread(new_arr[i].data, arr_size, new_arr[i].data_size, input) !=
            new_arr[i].data_size) {
            printf("error reading from file\n");
            exit(1);
        }
    ...
    for (int i = 0; i < struct_size; ++i) {
        ...
        assert(relative_addresses != NULL);
        if (fread(relative_addresses, sizeof(int), new_arr[i].num_edges,
                  input) != new_arr[i].num_edges) {
            printf("error reading from file\n");
            exit(1);
        }
...
void add_vertex(graph *in_graph, enum Types data_type, void *data,
    ...
    asser(in_graph->vertices[pos].data != NULL);
